...
class openTests(tests.common.common):
	def test_no_container_given_text(self):
		self.rpg.setAction([_('OPEN_COMMAND')])
	...
	def test_container_not_found_here_text(self):
		self.rpg.setAction([_('OPEN_COMMAND'), 'box'])
		output = self.rpg._runAction()
	...
	def test_unknown_container_type_text(self):
		self.rpg.setAction([_('OPEN_COMMAND'), 'some-container'])
		output = self.rpg._runAction()
	...
	def test_multiple_container_of_given_type_text(self):
		self.rpg.setAction([_('OPEN_COMMAND'), 'wardrobe'])
		output = self.rpg._runAction()
	...
	def test_container_number_provided_text(self):
		self.rpg.setAction([_('OPEN_COMMAND'), 'wardrobe', 0])
		output = self.rpg._runAction()
		self.assertEquals(output, _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (2,))

		self.rpg.setAction([_('OPEN_COMMAND'), 'chest', 1])
		output = self.rpg._runAction()
		expected = [
			_('ITEMS_IN_CONTAINER_%s') % 'chest',
			'  4 Heavy breastplate'
		...
		self.rpg.setAction([_('OPEN_COMMAND'), 'chest', 2])
		output = self.rpg._runAction()
		self.assertEquals(output, _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (1,))

		self.rpg.setAction([_('OPEN_COMMAND'), 'wardrobe', 1])
		output = self.rpg._runAction()
		expected = [
		...
		self.rpg.setAction([_('OPEN_COMMAND'), 'wardrobe', 2])
		output = self.rpg._runAction()
		expected = [
			_('ITEMS_IN_CONTAINER_%s') % 'wardrobe',
			'  4 Mist potion'
