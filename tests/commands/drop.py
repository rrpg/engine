...
class dropTests(tests.common.common):
	def test_no_item_given_text(self):
		with capturer() as output:
			...
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['drop'])
			rpg._runAction()
	...
	def test_unknown_item_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['drop', 'some dummy item'])
			rpg._runAction()
	...
	def test_item_not_here_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['drop', 'Mist potion'])
			rpg._runAction()
		self.assertTrue(output == ['I have none of those'])
	...
	def test_quantity_too_high_text(self):
		rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
		rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER')
