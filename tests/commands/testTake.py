...
class takeTests(tests.common.common):
	...
	def test_not_available_container_type_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'box'])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('ERROR_CONTAINER_NOT_AVAILABLE'))

	def test_not_available_container_type_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'box'])
		output = self.rpgJSON._runAction()
	...
	def test_missing_container_index_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe'])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('ERROR_MULTIPLE_CONTAINERS_AVAILABLE'))

	def test_missing_container_index_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe'])
		output = self.rpgJSON._runAction()
	...
	def test_invalid_container_index_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 'some index'])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('ERROR_TAKE_INVALID_CONTAINER_INDEX'))

	def test_invalid_container_index_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 'some index'])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"error": {"message": _('ERROR_TAKE_INVALID_CONTAINER_INDEX'), "code": 1}})

	def test_out_of_range_container_index_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 0])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (2,))
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 3])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (2,))
	...
	def test_out_of_range_container_index_json(self):
		...
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 0])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"error": {"message": _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (2,), "code": 1}})
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 3])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"error": {"message": _('ERROR_OUT_OF_RANGE_ITEM_CONTAINER_INDEX_%d') % (2,), "code": 1}})
	...
	def test_item_container_index_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 1])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('TAKE_CONFIRMATION_%(quantity)s_%(name)s') % {'quantity': 1, 'name': 'Heavy breastplate'})

	def test_item_container_index_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 'Heavy breastplate', 'wardrobe', 1])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"name": "Heavy breastplate", "quantity": 1})
	...
	def test_quantity_item_container_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 2, 'Heavy breastplate', 'chest'])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('TAKE_CONFIRMATION_%(quantity)s_%(name)s') % {'quantity': 2, 'name': 'Heavy breastplate'})

	def test_quantity_item_container_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 2, 'Heavy breastplate', 'chest'])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"name": "Heavy breastplate", "quantity": 2})

	def test_quantity_item_container_index_text(self):
		self.rpgText.setAction([_('TAKE_COMMAND'), 2, 'Heavy breastplate', 'wardrobe', 1])
		output = self.rpgText._runAction()
		self.assertTrue(output == _('TAKE_CONFIRMATION_%(quantity)s_%(name)s') % {'quantity': 2, 'name': 'Heavy breastplate'})

	def test_quantity_item_container_index_json(self):
		self.rpgJSON.setAction([_('TAKE_COMMAND'), 2, 'Heavy breastplate', 'wardrobe', 1])
		output = self.rpgJSON._runAction()
		self.assertTrue(output == {"name": "Heavy breastplate", "quantity": 2})
