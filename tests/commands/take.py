...
class takeTests(tests.common.common):
	def test_no_item_given_text(self):
		with capturer() as output:
			...
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take'])
			rpg._runAction()
	...
	def test_unknown_item_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take', 'some dummy item'])
			rpg._runAction()
	...
	def test_item_not_here_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take', 'Mist potion'])
			rpg._runAction()
		self.assertTrue(output == ['I don\'t see this here.'])
	...
	def test_quantity_too_high_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take', 10, 'Heavy breastplate'])
			rpg._runAction()
	...
	def test_with_quantity_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take', 2, 'Heavy breastplate'])
			rpg._runAction()
	...
	def test_implied_quantity_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['take', 'Heavy breastplate'])
			rpg._runAction()
