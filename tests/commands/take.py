...
class takeTests(tests.common.common):
	...
	def test_no_item_given_json(self):
		...
		self.assertTrue(json.loads(output[0]) == {"error": {"message": _('ERROR_TAKE_NO_ITEM_GIVEN'), "code": 1}})
	...
	def test_unknown_item_json(self):
		with capturer() as output:
			...
			self.rpgJSON._runAction()
		self.assertTrue(json.loads(output[0]) == {"error": {"message": _('ERROR_TAKE_UNKNOWN_ITEM'), "code": 1}})
	...
	def test_item_not_here_json(self):
		with capturer() as output:
			...
			self.rpgJSON._runAction()
		self.assertTrue(json.loads(output[0]) == {"error": {"message": _('ERROR_TAKE_ITEM_NOT_AVAILABLE'), "code": 1}})
	...
	def test_quantity_too_high_json(self):
		with capturer() as output:
			...
			self.rpgJSON._runAction()
		self.assertTrue(json.loads(output[0]) == {"error": {"message": _('ERROR_TAKE_QUANTITY_TOO_HIGH'), "code": 1}})
