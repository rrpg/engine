...
class moveTests(tests.common.common):
	def test_no_direction_given_text(self):
		with capturer() as output:
			...
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['move'])
			rpg._runAction()
	...
	def test_invalid_direction_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['move', 'bad-direction'])
			rpg._runAction()
	...
	def test_not_available_direction_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['move', 'east'])
			rpg._runAction()
	...
	def test_text(self):
		with capturer() as output:
			rpg = Rpg.Rpg(renderMode=Rpg.RENDER_TEXT)
			rpg.init(self.dbFile, 'TEST_PLAYER', 'TEST_PLAYER', ['move', 'north'])
			rpg._runAction()
